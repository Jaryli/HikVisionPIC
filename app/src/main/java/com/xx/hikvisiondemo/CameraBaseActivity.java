...
public class CameraBaseActivity extends Activity implements Callback, OnTouchListener{
    ...
    //设备的ip地址
    public final String ADDRESS = "192.168.1.150";
    //设备的端口号
    ...
    public void processRealData(int iDataType,
        ...
        if (!m_bNeedDecode) {
            ...
            if (HCNetSDK.NET_DVR_SYSHEAD == iDataType) {
                ...
                if (iDataSize > 0) {
                    if (!playerInstance.setStreamOpenMode(m_iPort,
                            iStreamMode)) // set stream mode
                    {
                        Log.e(TAG, "setStreamOpenMode failed");
                        return;
                    }
                    if (!playerInstance.openStream(m_iPort, pDataBuffer,
                            iDataSize, 2 * 1024 * 1024)) // open stream
                    {
                        Log.e(TAG, "openStream failed");
                        return;
                    }
                    if (!playerInstance.play(m_iPort,
                            m_osurfaceView.getHolder())) {
                        Log.e(TAG, "play failed");
                        return;
                    }
                    if (!playerInstance.playSound(m_iPort)) {
                ...
                if (!playerInstance.inputData(m_iPort, pDataBuffer,
                        iDataSize)) {
                    // Log.e(TAG, "inputData failed with: " +
                    // Player.getInstance().getLastError(m_iPort));
                    for (int i = 0; i < 4000 && m_iPlaybackID >= 0
                            && !m_bStopPlayback; i++) {
                        if (playerInstance.inputData(m_iPort,
